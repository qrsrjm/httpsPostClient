...
#define DEFAULT_SCRIPT	"/usr/share/udhcpc/default.script"        //ADD BY HWX 2015:04:15 STUDY
...
#endif
{
    ...
    // 如果没有使用clientid选项，则分配空间，用MAC地址填充
	...
	state = INIT_SELECTING;              // 当前状态: 初始选择阶段
	run_script(NULL, "deconfig");        // 运行脚本
	change_mode(LISTEN_RAW);             // RAM模式
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			switch (state) {
			case INIT_SELECTING:          // 当前处于初始选择阶段，发送发现广播包
				if (packet_num < 3) {
					if (packet_num == 0)
						xid = random_xid();
